...
async def generate_structure(topic: str) -> str:
    prompt = (
        f"Ты — преподаватель дисциплины «Сметное дело в строительстве».\n\n"
        ...
        f"- Введение\n"
        f"- Теоретические основы ценообразования в строительстве\n"
        f"- Характеристика объекта сметных расчётов\n"
        f"- Составление сметной документации\n"
        f"- Анализ сметной стоимости\n"
        f"- Заключение\n"
        f"- Список литературы\n\n"
...
async def generate_section_prompt(topic: str, section_number: str, section_title: str) -> str:
    return (
        f"Ты — инженер-сметчик. Напиши содержание раздела "
        f"«{section_title}» для курсового проекта на тему «{topic}».\n\n"
        f"Требования:\n"
        f"- Объём: 2-5 абзацев\n"
        f"- Используй профессиональную терминологию\n"
        f"- Ссылайся на МДС, ФЕР, ТЕР, ГЭСН, приказы Минстроя где уместно\n"
