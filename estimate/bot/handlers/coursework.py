...
TOPICS = [
    "Смета на строительство жилого дома",
    "Смета на ремонт помещения",
    "Локальный сметный расчёт (ЛСР) на СМР",
    "Объектный сметный расчёт (ОСР)",
    "Сводный сметный расчёт стоимости строительства (ССРСС)",
    "Проверка сметной документации",
    "Акт КС-2 и справка КС-3",
    "Смета по ФЕР",
    "Смета по ТЕР",
    "Расчёт накладных расходов и сметной прибыли",
    "Смета на отделочные работы",
    "Смета на монтаж инженерных систем",
...
async def cmd_coursework(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "📚 <b>Конструктор курсового проекта по сметному делу</b>\n\n"
...
async def pick_topic(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if topic == "Свой вариант (введу сам)":
        await callback.message.edit_text("Напиши свою тему курсового проекта по сметному делу:")
...
async def process_section(message: types.Message, state: FSMContext) -> None:
    ...
    section = matched[0]
    remaining = [s["number"].strip().lower().split()[0].rstrip(".") for s in matched[1:]]
