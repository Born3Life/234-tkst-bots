...
async def generate_structure(topic: str) -> str:
    prompt = (
        f"Ты — преподаватель дисциплины «Строительно-монтажные работы».\n\n"
        ...
        f"- Введение\n"
        f"- Характеристика объекта строительства\n"
        f"- Технология и организация работ\n"
        f"- Контроль качества и испытания\n"
        f"- Охрана труда и техника безопасности\n"
        f"- Календарный график производства работ\n"
        f"- Заключение\n"
        f"- Список литературы\n\n"
...
async def generate_section_prompt(topic: str, section_number: str, section_title: str) -> str:
    return (
        f"Ты — инженер ПТО. Напиши содержание раздела "
        f"«{section_title}» для курсового проекта на тему «{topic}».\n\n"
        f"Требования:\n"
        f"- Объём: 2-5 абзацев\n"
        f"- Используй профессиональную терминологию\n"
        f"- Ссылайся на СП, СНиП, ГОСТ, приказы Минтруда где уместно\n"
