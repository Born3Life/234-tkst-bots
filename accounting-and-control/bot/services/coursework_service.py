...
async def generate_structure(topic: str) -> str:
    prompt = (
        ...
        f"- Введение\n"
        f"- Теоретические основы учёта и контроля\n"
        f"- Анализ деятельности организации\n"
        f"- Практическая часть\n"
        f"- Заключение\n"
        f"- Список литературы\n\n"
...
async def generate_section_prompt(topic: str, section_number: str, section_title: str) -> str:
    return (
        f"Ты — специалист по учёту и контролю в строительстве. Напиши содержание раздела "
        f"«{section_title}» для курсового проекта на тему «{topic}».\n\n"
        f"Требования:\n"
        f"- Объём: 2-5 абзацев\n"
        f"- Используй профессиональную терминологию\n"
        f"- Ссылайся на нормативные документы где уместно\n"
