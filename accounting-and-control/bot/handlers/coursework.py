...
TOPICS = [
    "Учёт строительных материалов на складе",
    "Учёт затрат на строительное производство",
    ...
    "Документальное оформление СМР",
    "Учёт основных средств в строительстве",
    "Контроль сметной стоимости строительства",
...
async def cmd_coursework(message: types.Message, state: FSMContext) -> None:
    ...
    await message.answer(
        "📚 <b>Конструктор курсового проекта по учёту и контролю</b>\n\n"
...
async def pick_topic(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if topic == "Свой вариант (введу сам)":
        await callback.message.edit_text("Напиши свою тему курсового проекта по учёту и контролю:")
